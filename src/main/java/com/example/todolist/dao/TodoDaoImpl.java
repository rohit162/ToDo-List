...
public class TodoDaoImpl implements TodoDao {
	...
	private static final String INSERT_TODOS_SQL = "INSERT INTO todos"
			+ "  (title, username, description, target_date,  isdone) VALUES " + " (?, ?, ?, ?, ?);";

	private static final String SELECT_TODO_BY_ID = "select id,title,username,description,target_date,isdone from todos where id =?";
	private static final String SELECT_ALL_TODOS = "select * from todos";
	private static final String DELETE_TODO_BY_ID = "delete from todos where id = ?;";
	private static final String UPDATE_TODO = "update todos set title = ?, username= ?, description =?, target_date =?, isdone = ? where id = ?;";
	...
	public void insertTodo(Todo todo) throws SQLException {
		...
		try (Connection connection = JDBCUtlis.getConnection();
			...
			preparedStatement.setDate(4, JDBCUtlis.getSQLDate(todo.getTargetDate()));
			preparedStatement.setBoolean(5, todo.getStatus());
			System.out.println(preparedStatement);
			preparedStatement.executeUpdate();
		} catch (SQLException exception) {
			JDBCUtlis.printSQLException(exception);
		}
	}
	...
	public boolean updateTodo(Todo todo) throws SQLException {
		...
		try (Connection connection = JDBCUtlis.getConnection();
			...
			statement.setDate(4, JDBCUtlis.getSQLDate(todo.getTargetDate()));
